...
function CosmosValidatorRow({ validator, active, onClick, unit, currency }: Props) {
  const onExternalLink = useCallback(() => {
    const url = "https://atomscan.com/validators/" + validator.validatorAddress;
        ...
        <Box style={{ flexDirection: "row" }}>
          <Box style={{ flexDirection: "column" }}>
            <Text ff="Inter|SemiBold" color="palette.text.shade100" fontSize={4}>
              {formatCurrencyUnit(unit, new BigNumber(validator.votingPower * 100), {
                showCode: true,
              })}
            </Text>
            <Text fontSize={2} textAlign="right">
              <Trans color="palette.text.shade50" i18nKey="solana.delegation.totalStake" />
            </Text>
          </Box>
      ...
      subtitle={
        <Box>
          <Text ff="Inter|Medium" fontSize={2} color="palette.text.shade50">
            <Trans i18nKey="solana.delegation.commission" /> {`${validator.commission * 100} %`}
          </Text>
        </Box>
      }
    ></StyledValidatorRow>
